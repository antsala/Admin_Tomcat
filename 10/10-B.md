...
Para que podamos conectar desde cualquier IP, necesitamos que esta etiqueta no se procese. La forma más sencilla es comentarla en xml, como podemos ver en la siguiente imagen indicada por los rectángulos en color rojo.
...
Por lo tanto, cuando ves la configuración "Valve className="org.apache.catalina.valves.RemoteAddrValve"", significa que se está utilizando la válvula `RemoteAddrValve` para filtrar las solicitudes basadas en la dirección IP remota del cliente en Tomcat.
