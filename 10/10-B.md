...
5. **Empaqueta tu aplicación en un archivo WAR**:

Si estás utilizando Maven, ejecuta el siguiente comando en la línea de comandos dentro del directorio raíz de tu proyecto:

```
mvn package
```

Esto generará un archivo WAR en el directorio `target` de tu proyecto.

¡Listo! Ahora has creado una aplicación web Java muy simple y has generado un archivo WAR para ella. Este archivo WAR puede ser implementado en cualquier contenedor de servlets compatible con Java EE, como Apache Tomcat.
